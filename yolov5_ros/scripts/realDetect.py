...
from utils.general import check_img_size, check_requirements, non_max_suppression, apply_classifier, scale_boxes, xyxy2xywh, strip_optimizer, set_logging, increment_path #from utils.general import check_img_size, check_requirements, non_max_suppression, apply_classifier, scale_coords, \
#    xyxy2xywh, strip_optimizer, set_logging, increment_path
from utils.plots import plot_one_box #save_one_box #plot_one_box as save_one_box # as save_one_box hinzugefügt
from utils.torch_utils import select_device, reshape_classifier_output, time_sync #from utils.torch_utils import select_device, load_classifier, time_synchronized
...
def detect(save_img=False):
    ...
    #for path, img, im0s, vid_cap in dataset:
        ...
        # Inference
        t1 = time_sync() #t1 = time_synchronized()
        pred = model(img, augment=opt.augment)[0]

        # Apply NMS
        pred = non_max_suppression(pred, opt.conf_thres, opt.iou_thres, classes=opt.classes, agnostic=opt.agnostic_nms)
        t2 = time_sync() #t2 = time_synchronized()
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                det[:, :4] = scale_boxes(img.shape[2:], det[:, :4], im0.shape).round() #det[:, :4] = scale_coords(img.shape[2:], det[:, :4], im0.shape).round()
